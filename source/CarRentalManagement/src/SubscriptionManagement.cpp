...
using namespace std;
void extendRental(vector<Car> &cars, vector<int> &amount, string brandName,
		string num) {
	int flag;
	int day;
	string name, _name;
	do {
		...
		if (flag == 1) {
			cout << "How many days you want to extend the rental period?"
					<< endl;
			cin >> day;
			...
			cout << "Invalid credentials,you want to try again?(yes/no)"
					<< endl;
			cin >> _name;
		}
	} while (_name == "yes");
...
void returnCar(vector<Car> &cars, vector<int> &amount, string brandName,
		string regNum) {
	int flag;
	...
	do {
		...
		if (choice == "yes") {
			...
			if (flag == 1) {
				...
				cout
						<< "The given credentials are not valid to return the car,you want to try again?(yes/no)"
						<< endl;
				cin >> _choice;
				if (_choice == "no") {
...
void rentCar(vector<Car> &cars, vector<Car> &rentedCar, vector<int> &amount,
	...
	if ((decide == "yes") || (decide == "no")) {
		...
		if (option == "yes") {
			if (subscriberLevel == "Silver") {
				cout
						<< "Please upgrade your subscription to gold or platinum to rent another car."
						<< endl;
