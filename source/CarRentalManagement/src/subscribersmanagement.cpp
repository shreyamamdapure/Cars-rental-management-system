...
void CarRentalSystem::addSubscriber(const std::string &name,
		const std::string &id, SubscriptionLevel level) {
	int maxCars;
	double discount;
	...
	for (auto &subscriber : subscribers) {
		if (subscriber.getID() == id) {
			...
			if (currentLevel == level) {
				std::cout << "Subscriber with ID " << id << " is already at "
						<< levelToString(level) << " level." << std::endl;
				return;
			}

			subscriber.setLevel(level);
			std::cout << "Subscriber with ID " << id << " promoted to "
					<< levelToString(level) << " level." << std::endl;
			return;
		}
	}
